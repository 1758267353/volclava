 ...
 * Similar to send_batch but handles multiple jobs
 */
static LS_LONG_INT
          ...
          int jobCount, struct submitReply *submitReply, struct lsfAuth *auth)
{
     ...
     * Follow LSF standard pattern: base_struct + dynamic_data + overhead */
    reqBufSize = sizeof(struct packSubmitReq) + 
                 jobCount * sizeof(struct submitReq) + 
                 jobCount * 2048 +  /* Estimated per-job data */
                 xdr_lsfAuthSize(auth) + 100;  /* XDR overhead */
    /* Cap the buffer size to prevent excessive memory usage */
    if (reqBufSize > DEF_LSB_MAX_PACK_JOBS * sizeof(struct submitReq) + 
                     DEF_LSB_MAX_PACK_JOBS * 2048 + 
                     sizeof(struct packSubmitReq) + 1024) {
        reqBufSize = DEF_LSB_MAX_PACK_JOBS * sizeof(struct submitReq) + 
                     DEF_LSB_MAX_PACK_JOBS * 2048 + 
                     sizeof(struct packSubmitReq) + 1024;
    }
    if ((request_buf = (char *) malloc(reqBufSize)) == NULL) {
...
cleanup:
    ...
    if (homeDir) {
        for (i = 0; i < jobCount; i++)
            if (homeDir[i]) free(homeDir[i]);
        free(homeDir);
    }
    if (resReq) {
        for (i = 0; i < jobCount; i++)
            if (resReq[i]) free(resReq[i]);
        free(resReq);
    }
    if (cmd) {
        for (i = 0; i < jobCount; i++)
    ...
    if (jf_array) {
        for (i = 0; i < jobCount; i++)
            if (jf_array[i].data) free(jf_array[i].data);
        free(jf_array);
    }
    if (packReq.jobs) free(packReq.jobs);
