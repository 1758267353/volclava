...
do_sub (int argc, char **argv, int option)
{
    ...
    if (req.options & SUB_PACK) {
          /* Use simplified pack submission feature */
        jobId = do_pack_sub_v2(option, argv, &req);
        if (jobId < 0) {
            fprintf(stderr, ". %s.\n",
...
LS_LONG_INT do_pack_sub_v2(int option, char **argv, struct submit *req)
{
    ...
    struct submit *job_requests[DEF_LSB_MAX_PACK_JOBS];  
    struct lenData *job_files[DEF_LSB_MAX_PACK_JOBS];  
    int job_count = 0;
    int max_jobs = DEF_LSB_MAX_PACK_JOBS;
    
  
    char *max_jobs_env = getenv("LSB_BATCH_MAX_JOBS");
    if (max_jobs_env) {
        max_jobs = atoi(max_jobs_env);
        if (max_jobs <= 0 || max_jobs > DEF_LSB_MAX_PACK_JOBS) {
            max_jobs = DEF_LSB_MAX_PACK_JOBS;
        }
    }
    
    /* Validate against configuration file setting */
    if (lsbMaxPackJobs > 0 && max_jobs > lsbMaxPackJobs) {
        fprintf(stderr, "Warning: Requested %d jobs exceeds LSB_MAX_PACK_JOBS limit (%d). Using limit.\n", 
                max_jobs, lsbMaxPackJobs);
        max_jobs = lsbMaxPackJobs;
    }
    ...
    while ((line = getNextLineC_(fp, &lineNum, TRUE)) != NULL && job_count < max_jobs) {
        ...
        for(int loop = 0; loop < packReq.numAskedHosts; loop++) {
            subNewLine_(packReq.askedHosts[loop]);
        }
    ...
    fprintf(stderr, "Successfully collected %d jobs, starting pack submission...\n", job_count);
    ...
    if (send_batch_pack(job_requests, job_files, job_count) < 0) {
        ...
        for (int i = 0; i < job_count; i++) {
            if (job_requests[i]) {
                ...
                if (job_requests[i]->jobName) free(job_requests[i]->jobName);
                if (job_requests[i]->packFile) free(job_requests[i]->packFile);
                if (job_requests[i]->queue) free(job_requests[i]->queue);
                if (job_requests[i]->resReq) free(job_requests[i]->resReq);
                if (job_requests[i]->hostSpec) free(job_requests[i]->hostSpec);
                if (job_requests[i]->dependCond) free(job_requests[i]->dependCond);
                if (job_requests[i]->inFile) free(job_requests[i]->inFile);
                if (job_requests[i]->outFile) free(job_requests[i]->outFile);
                if (job_requests[i]->errFile) free(job_requests[i]->errFile);
                if (job_requests[i]->command) free(job_requests[i]->command);
                if (job_requests[i]->newCommand) free(job_requests[i]->newCommand);
                if (job_requests[i]->chkpntDir) free(job_requests[i]->chkpntDir);
                if (job_requests[i]->preExecCmd) free(job_requests[i]->preExecCmd);
                if (job_requests[i]->postExecCmd) free(job_requests[i]->postExecCmd);
                if (job_requests[i]->mailUser) free(job_requests[i]->mailUser);
                if (job_requests[i]->projectName) free(job_requests[i]->projectName);
                if (job_requests[i]->loginShell) free(job_requests[i]->loginShell);
                if (job_requests[i]->askedHosts) {
                    for (int j = 0; j < job_requests[i]->numAskedHosts; j++) {
                        if (job_requests[i]->askedHosts[j]) free(job_requests[i]->askedHosts[j]);
                    }
                    free(job_requests[i]->askedHosts);
                }
                if (job_requests[i]->xf) free(job_requests[i]->xf);
                free(job_requests[i]);
            }
            if (job_files[i]) {
                if (job_files[i]->data) {
                    free(job_files[i]->data);
                }
                free(job_files[i]);
            }
        }
...
static int send_batch_pack(struct submit **job_requests, struct lenData **job_files, int job_count)
{
    ...
    fprintf(stderr, "========================================\n");
    fprintf(stderr, "Start pack submission (%d jobs)...\n", job_count);
    fprintf(stderr, "========================================\n");
    ...
    fprintf(stderr, "========================================\n");
    fprintf(stderr, "Submitted job count: %d\n", job_count);
    if (firstJobId > 0) {
        fprintf(stderr, "First job ID: %lld\n", (long long)firstJobId);
        if (job_count > 1) {
            fprintf(stderr, "Job ID range: %lld - %lld\n", 
                    (long long)firstJobId,
                    (long long)(firstJobId + job_count - 1));
        }
    }
